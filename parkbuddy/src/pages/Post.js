...
import { Card, CardImg, CardBody, CardTitle, CardSubtitle, Button, Container, Input, Form } from 'reactstrap';
import './post.css';
...
class Post extends Component {
	...
	addComment = (comment, id, username) => {
		...
		axios
			.post(
				...
				{ comment: this.state.comment, repliedBy: this.state.repliedBy, post_id: this.state.post_id },
	...
	updateComment = (comment, id) => {
		...
		axios
			.put(
				...
				{ comment: this.state.comment, repliedBy: this.state.repliedBy },
				{ headers: { Authorization: token } }
			)
			.then((res) => {
				this.props.getPosts();
	...
	deleteComment = (e, id) => {
		...
		const token = localStorage.getItem('token');
		axios
			.delete(`https://disney-parent.herokuapp.com/api/posts/comment/${id}`, {
				headers: { Authorization: token }
			})
			.then((res) => {
				this.props.getPosts();
		...
		return (
			<div>
				{this.state.isEditing ? (
					...
					this.updateComment(this.state.comment, this.state.id)
				)}
			...
			<Container className="wrap">
				<div>
					<Card className="shadow border">
						<CardTitle onClick={(e) => this.getPost(e, this.props.post.id)} className="center large">
						...
						<CardBody className="teal">
							<CardTitle className="strong">
								Username:<span>&nbsp;</span> {this.props.post.postedBy}{' '}
							</CardTitle>
							<CardSubtitle className="pad">
								<strong>
									Date: <span>&nbsp;</span>
								</strong>{' '}
							...
							<CardSubtitle className="pad">
								...
								We are located in {this.props.post.meetingPlace} with {this.props.post.numOfKids}{' '}
								children
							</CardSubtitle>
							...
							<Form onSubmit={this.submitHandler}>
								...
								{this.state.isEditing ? (
									...
									<div className="centered">
										<Button
											className="blue smaller centered"
											onClick={this.toggle(this.state.comment.id)}
										>
											Cancel
										</Button>
										<Input
						...
						<div className="displayFlex">
							{this.props.post.user_id === thisId ? (
								<Button
									className="leftButton red"
									onClick={(e) => this.deletePost(e, this.props.post.id)}
								>
									Delete Post
								</Button>
							) : null}
							{this.props.post.user_id === thisId ? (
								<Button
									className="rightButton light-blue"
									onClick={(e) => this.updatePost(e, this.props.post.id)}
								>
									Update Post
								</Button>
							) : null}
